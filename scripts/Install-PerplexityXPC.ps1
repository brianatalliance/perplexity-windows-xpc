...
.PARAMETER SkipDefenderExclusions
    Do not automatically add Windows Defender and ASR exclusions.
    Use this in environments where exclusions are managed via Intune,
    Group Policy, or a separate IT process.
...
param(
    ...
    [Parameter(HelpMessage = "Skip automatic Defender/ASR exclusion setup. Use this when your environment manages Defender policy separately (e.g. via Intune or GPO).")]
...
#region Unblock files and add Defender exclusions
# ---------------------------------------------------------------------------
# This must run BEFORE pre-flight checks so Defender does not interfere with
# the binary checks and process launches that follow.
# ---------------------------------------------------------------------------
Write-Step "Removing MOTW flags and configuring Defender exclusions..."

# -- Unblock all scripts in the scripts\ directory --
try {
    $scriptDir = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Definition }
    Get-ChildItem -Path $scriptDir -Recurse -File -ErrorAction SilentlyContinue |
        Unblock-File -ErrorAction SilentlyContinue
    Write-Success "Unblocked scripts in: $scriptDir"
} catch {
    Write-Warn "Unblock pass encountered an issue: $_"
}

# -- Optionally delegate to Unblock-PerplexityXPC.ps1 for a broader pass --
try {
    $unblockScript = Join-Path $scriptDir 'Unblock-PerplexityXPC.ps1'
...
# -- Add Defender / ASR exclusions --
if (-not $SkipDefenderExclusions) {
    $defenderScript = Join-Path $scriptDir 'Add-DefenderExclusions.ps1'
    if (Test-Path $defenderScript) {
        try {
            & $defenderScript -InstallPath $InstallPath -ErrorAction SilentlyContinue
        } catch {
            Write-Warn "Add-DefenderExclusions.ps1 reported: $_"
        }
    } else {
        # Inline fallback: add exclusions directly
        $mpAvailable = $null -ne (Get-Command 'Add-MpPreference' -ErrorAction SilentlyContinue)
        if ($mpAvailable) {
            ...
            Write-Success "Defender exclusions added for: $InstallPath"
        } else {
            Write-Warn 'Add-MpPreference not available - Defender exclusions skipped.'
        }
    }
} else {
    Write-Status '  [SKIP] Defender exclusions skipped (-SkipDefenderExclusions specified).' -Color DarkGray
}
#endregion
